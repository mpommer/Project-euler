...
m <- matrix(, nrow <- 2000, ncol <- 10)


# split up vector in columns
hands_one_r <- strsplit(hands_all,"\n")[[1]]


# create vector with sinlge cards
vector <- ""
for(i in 1:1000){
hands_one_g <- strsplit(hands_one_r[i]," ")[[1]]
vector <- append(vector, hands_one_g)
}
vector <- vector[2:10001]


# create vecotr with part of card (number and sign)
vector_final <- ""
for(i in 1:10000){
single <- strsplit(vector[i],"")[[1]]
vector_final <- append(vector_final, single)
}
vector_final <- vector_final[2:20001]



# change t,j,k,a into numbers
for(i in 1:20000){
  if(vector_final[i]=="T"){
    vector_final[i] <- 10
  }
...
# fill numbers in matrix
n <- 1
for(i in seq(1,2000, by=2)){
  for(j in 1:10){
    m[i,j] <- vector_final[n]
    n <- n+2
  }
}
